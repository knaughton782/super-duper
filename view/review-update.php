...
$reviewList = '<select name="reviewId" id="reviewId" required>';
$reviewList .= '<option>Select a review to update: </option>';

foreach ($reviews as $review) {
    $reviewList .= "<option value='$review[reviewId]' ";
    if (isset($reviewId)) {

        if ($review['reviewId'] === $reviewId) {
            $reviewList .= ' selected ';
        }
    } elseif (isset($prodInfo['reviewId'])) {
        if ($review['reviewId'] === $prodInfo['reviewId']) {
            $reviewList .= ' selected ';
        }
    }
    $reviewList .= ">$review[categoryName]</option>";
}
$reviewList .= '</select>';
...
<main id="page-content"> 
    ...
    <h1><?php
        ...
        ?></h1>
        <h2>Use this form to edit your product reviews. All fields are required.</h2><br><br>
    <?php
